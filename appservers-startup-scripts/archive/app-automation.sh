...
aws s3 cp s3://prod-proxy-app-db-config-omari/dbinfo.inc .
...
#6. Create a new file in the ``html`` directory named OmariMailingApp.php, 
#   and then edit the file by calling ``vi`` (or the editor of your choice).
vi OmariMailingApp.php

#7. Add the following contents to the ``OmariMailingApp.php`` file. COPY APP CODE FROM THE PROJECT
# Use the OmariMailingApp.php file from the app-web-db-config folder

#8. Save and close the OmariMailingApp.php file

#9. Verify that your web server successfully connects to your DB instance by opening a web browser and browsing to
Access Application: http://BACKEND_LOAD_BALANCER_DNS/OmariMailingApp.php
